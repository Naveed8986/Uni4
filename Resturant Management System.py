...
root.title("Nav's BBQ Chicken Shack")

text_Input = StringVar()
operator = ""

Tops = Frame(root, width=1600, height=50, bg="skyblue", relief=SUNKEN)
Tops.pack(sid=TOP)
...
lblInfo = Label(Tops, font=('arial', 50, 'bold'),
                text="Nav's BBQ Chicken Shack", fg="Steel Blue",
                bd=10, anchor='w')
...
def Ref():
    ...
    CostofMeal = "GBP", str('%.2f' % (CostofFries + CostofDrinks + CostofFilet + CostofBurger
    ...
    Service = "GBP", str('%.2f' % Ser_Charge)

    OverAllCost = "GBP", str('%.2f' % (PayTax + TotalCost + Ser_Charge))
    PaidTax = "GBP", str('%.2f' % PayTax)
